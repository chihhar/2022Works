...
class omi(tf.keras.Model):
    #-------
    def __init__(self, visual_path='visualization', baseChn=16, temp_feature_size=64, nn_size=64, size_layer_chfn=2, hidden_num=30, isNormalize=True):
    ...
    def build(self, input_shape):
        ...
        self.cumlative_hazard_function = cumlative_hazard_function(nn_size=self.nn_size,  size_layer_chfn=self.size_layer_chfn)
    #-------
    ...
    def call(self, x):
        ...
        #---------
        # cumulative hazard function
        cum_hazard_tau = self.cumlative_hazard_function(diff_T_future_nmlz, past_hidden_fc)
        #---------

        return cum_hazard_tau, diff_T_future
    ...
    def train_step(self, x):
        ...
        with tf.GradientTape() as tape1, tf.GradientTape() as tape2, tf.GradientTape() as tape3, tf.GradientTape() as tape4:
            ...
            #predict
            cum_hazard_tau, diff_T_future = self(x, training=True)
    ...
    def test_step(self, x):
        ...
        with tf.GradientTape() as tape1, tf.GradientTape() as tape2:
            ...
            #predict
            cum_hazard_tau, diff_T_future = self(x, training=False)
            tape1.watch(diff_T_future)
...
def plotT(true_hist, pred_hist, label='FT-PP'):
    ...
    plt.savefig(f'{visual_dir}/{"seismic" if isEarthquake else "toy"}_512_tau_{label}.pdf', bbox_inches='tight', pad_inches=0)
#----------------------------
...
def main():
    ...
    # モデル形成
    model = omi(visual_path='visualization')
